...
def mock_interface_stats(*args):
    return """_uuid               : 583d9020-a49a-4c5d-902d-dfcaa41e2911
admin_state         : up
bfd                 : {}
bfd_status          : {}
cfm_fault           : []
...
class TestDataif_Collection(TestCase):
    ...
    def setUp(self):
        ...
        self.port1 = self.fx_pmd_1.add_port("port1")
        self.port1.numa_id = self.fx_pmd_1.numa_id

        # add rxq to port object
        self.rxq_1 = self.port1.add_rxq(0)
        ...
        # add port object into pmd.
        self.port2 = self.fx_pmd_2.add_port("port2")
        self.port2.numa_id = self.fx_pmd_2.numa_id

        # add rxq to port object
        self.rxq_2 = self.port2.add_rxq(0)
    ...
    def test_get_pmd_stats_1(self):
        ...
        self.assertEqual(out_pmd_1.__eq__(expected_pmd_1), True)
        self.assertEqual(out_pmd_2.__eq__(expected_pmd_2), True)

    # Test case:
    #   getting interface stats from get_interface_stats function and checking if
    #   declared port objects are modified or not
    ...
    def test_get_interface_stats_1(self):
        ...
        expected = copy.deepcopy(self.port1)

        # modify these to expected
        expected.type = "dpdkvhostuserclient"
        expected.tx_retry_cyc[expected.cyc_idx] = 0
        ...
        self.assertEqual(self.port1.__eq__(expected), True)

        # create a copy of original port2
        expected = copy.deepcopy(self.port2)
        ...
        self.assertEqual(self.port2.__eq__(expected), True)
