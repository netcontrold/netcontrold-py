...
class Dataif_Coverage(object):
    """
    Class to represent the slowpath counters.
...
def get_coverage_stats(coverage_map):
    """
    Collect stats of upcall coverage counter. In every sampling iteration, these stats are stored
    ...
    cmd = "../ovs/utilities/ovs-appctl coverage/show"
...
def get_pmd_stats(pmd_map):
    ...
    # retrieve required data from the vswitch.
    cmd = "../ovs/utilities/ovs-appctl dpif-netdev/pmd-stats-show"
    data = util.exec_host_command(cmd)
    if not data:
        raise OsCommandExc("unable to collect data")
...
def get_pmd_rxqs(pmd_map):
    ...
    # retrieve required data from the vswitch.
    cmd = "../ovs/utilities/ovs-appctl dpif-netdev/pmd-rxq-show"
    data = util.exec_host_command(cmd)
    if not data:
        raise OsCommandExc("unable to collect data")
...
def get_port_stats():
    ...
    # retrieve required data from the vswitch.
    cmd = "../ovs/utilities/ovs-appctl dpctl/show -s"
    data = util.exec_host_command(cmd)
    if not data:
        raise OsCommandExc("unable to collect data")
...
def get_interface_stats():
    ...
    # retrieve required data from the vswitch.
    cmd = "../ovs/utilities/ovs-vsctl list interface"
    data = util.exec_host_command(cmd)
    if not data:
        raise OsCommandExc("unable to collect data")
